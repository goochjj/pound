...
DAEMON=/usr/sbin/sapphirepound19
DESC="reverse proxy and load balancer"
NAME=sapphirepound19
...
if [ -f "/etc/default/sapphirepound19" ]
then
  . /etc/default/sapphirepound19
  if [ "$startup" != "1" ]
  then
    log_warning_msg "$NAME will not start unconfigured."
    log_warning_msg "Please configure; afterwards, set startup=1 in /etc/default/sapphirepound19."
    exit 0
  fi
else
  log_failure_msg "/etc/default/sapphirepound19 not found"
  exit 1
fi
