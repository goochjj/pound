	...
	unsigned int len;
{
	...
	/* Demangle URLEncoded values */
	fromp = from; top = from; int startlen = len;
	for (; fromp<(from+startlen);) {
		*top = *fromp;
		if (*fromp++ == '%' && (i+2)<startlen) {
			if (fromp[0] == '2' && (fromp[1]=='B' || fromp[1]=='b' )) *top = '+';
			else if (fromp[0] == '2' && (fromp[1]=='F' || fromp[1]=='f' )) *top = '/';
			else if (fromp[0] == '3' && (fromp[1]=='D' || fromp[1]=='d' )) *top = '=';
			else return -1;
			fromp++; len--;
			fromp++; len--;
		}
